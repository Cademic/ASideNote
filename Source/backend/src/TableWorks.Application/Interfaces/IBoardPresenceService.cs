...
public interface IBoardPresenceService
{
    ...
    void AddPresence(Guid boardId, string connectionId, Guid userId, string displayName, string role, string? profilePictureKey);
    ...
    /// <summary>Get current presence for a board: list of (userId, displayName, role, profilePictureKey) excluding duplicates.</summary>
    IReadOnlyList<(Guid UserId, string DisplayName, string Role, string? ProfilePictureKey)> GetPresence(Guid boardId);
