...
export function Sidebar({
  ...
  return (
    ...
    >
      {/* Everything scrolls as one column; sections keep their natural height
          (shrink-0) so they never collapse into / overlap each other. The rail
          and collapse toggle stay outside this wrapper so they don't scroll. */}
      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto overflow-x-hidden scrollbar-thin">
        {/* Brand — logo + close (drawer) */}
        <div
          className={[
            "sidebar-brand flex h-14 shrink-0 items-center px-4",
          ...
          >
            <img
              ...
              className={[
                ...
                expanded ? "h-12 w-auto" : "h-14 w-14",
              ].join(" ")}
            />
          </Link>
          {isDrawer && (
        ...
        {onRequestCreate && (
          <div
            className={
              expanded
                ? "shrink-0 px-3 pt-3"
                : "flex shrink-0 justify-center pt-3"
            }
          >
            <button
              type="button"
        ...
        {(pinnedProjects.length > 0 ||
          ...
          <div className="flex shrink-0 flex-col border-t border-[var(--land-rule)]">
            <div className="flex flex-col gap-0.5 px-3 py-2">
              ...
              {filteredOpenedBoards.map((board) => {
                ...
                if (!fullBoard) {
                  ...
                  const Icon =
                    BOARD_TYPE_ICON[board.boardType] ?? ClipboardList;
                  return (
                    <div
                      key={board.id}
                      className={[
        ...
        {isOnAnyBoardPage && (
          <div className="shrink-0 border-t border-[var(--land-rule)] p-3">
            {expanded && (
            ...
            <div className="flex flex-col gap-0.5">
              {(isOnChalkBoardPage
                ...
                <button
                  ...
                  title={tool.label}
                  aria-label={expanded ? undefined : `Add ${tool.label}`}
                  className={[
                    "flex w-full cursor-pointer items-center gap-3 rounded-lg px-3 py-2 text-left text-sm font-medium text-[var(--land-ink-2)] transition-colors duration-150 hover:bg-[var(--land-cream)] hover:text-[var(--land-ink)] motion-reduce:transition-none",
                    !expanded && "justify-center",
                  ]
                    .filter(Boolean)
                    .join(" ")}
