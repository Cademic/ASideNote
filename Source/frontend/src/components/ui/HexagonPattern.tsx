...
/** Adapted from https://magicui.design/docs/components/hexagon-pattern (no shadcn/`cn` dependency in this repo). */
interface HexagonPatternProps extends SVGProps<SVGSVGElement> {
  /** Radius of each hexagon (center to vertex). @default 40 */
  radius?: number;
  /** Spacing in pixels between adjacent hexagons. @default 0 */
  gap?: number;
  /** Offset applied to the pattern origin on the x-axis. @default -1 */
  x?: number;
  /** Offset applied to the pattern origin on the y-axis. @default -1 */
  y?: number;
  /** "horizontal" = flat-top hexagons, "vertical" = pointy-top hexagons. @default "horizontal" */
  direction?: "horizontal" | "vertical";
  /** SVG stroke-dasharray applied to each hexagon outline. @default "0" */
  strokeDasharray?: string;
  /** [col, row] coordinates of hexagons to highlight (filled) on top of the pattern. */
...
function hexPoints(cx: number, cy: number, r: number, direction: "horizontal" | "vertical"): string {
