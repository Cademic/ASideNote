...
/** Adapted from https://magicui.design/docs/components/striped-pattern (no shadcn/`cn` dependency in this repo). */
interface StripedPatternProps extends SVGProps<SVGSVGElement> {
  /** Diagonal stripe direction. @default "left" */
  direction?: "left" | "right";
  /** Tile width in pixels. @default 10 */
  width?: number;
  /** Tile height in pixels. @default 10 */
  height?: number;
  className?: string;
}
...
export function StripedPattern({
  ...
  return (
    <svg
      aria-hidden="true"
      className={`pointer-events-none absolute inset-0 h-full w-full stroke-[0.5] ${className}`}
