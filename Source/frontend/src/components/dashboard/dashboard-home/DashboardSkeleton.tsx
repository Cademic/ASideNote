...
/**
 * Loading placeholder for {@link DashboardLayout}. Matches its structure — the
 * "Projects + day schedule" column — so the page doesn't jump when real data
 * arrives. Uses the shared `.skeleton` shimmer class.
 */
export function DashboardSkeleton() {
  return (
    <div
      className="flex w-full min-w-0 flex-col lg:h-full lg:min-h-0 lg:overflow-hidden"
      aria-busy="true"
      aria-label="Loading your workspace"
    >
      {/* Greeting, projects tree, day schedule */}
      <div className="lg:h-full lg:min-h-0 lg:overflow-hidden">
        <div className="flex w-full flex-col bg-background lg:h-full lg:min-h-0 lg:flex-row">
          ...
          {/* Projects — left column on desktop */}
